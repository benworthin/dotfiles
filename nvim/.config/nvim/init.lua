...
-- Configure tabs to be 2 spaces and convert to spaces
...
-- Tab Navigation
--  Use <leader>t + hjkl to manage tabs
--  See `:help tabnew`, `:help tabnext`, `:help tabprevious`
-- Barbar buffer navigation (b prefix)
vim.keymap.set("n", "<leader>bk", "<cmd>BufferNext<CR>", { desc = "[B]uffer [K]next (right)" })
vim.keymap.set("n", "<leader>bj", "<cmd>BufferPrevious<CR>", { desc = "[B]uffer [J]previous (left)" })
vim.keymap.set("n", "<leader>bc", "<cmd>BufferClose<CR>", { desc = "[B]uffer [C]lose" })
vim.keymap.set("n", "<leader>bn", "<cmd>enew<CR>", { desc = "[B]uffer [N]ew" })

-- Vim tab navigation (t prefix)
...
vim.keymap.set("n", "<leader>zf", function()
	vim.cmd("normal vafzf")
end, { noremap = true, desc = "[Z]ip [F]old current function" })

-- Go Dev Keymaps
vim.keymap.set("n", "<leader>gi", ":!goimports -w %<CR>:e<CR>", { desc = "Run goimports on the current file" })
...
require("lazy").setup({
	...
	{ -- Better landing page with options when opening Neovim without specifying a file.
		"goolord/alpha-nvim",
		event = "VimEnter",
		config = function()
